...
% *************************************************************************************************************
    ...
    \subsection{東側外縁部に対する評価}
      ...
      \subsubsection{予測対実測散布図による定量的評価}
        ...
        これらの結果から、動画予測モデルは、一定の複雑さを持つ東側外縁部の平均輝度に対しても、高い再現能力を持つことがわかった。
        \begin{figure}[htbp]
    ...
    \subsection{まとめ}
      ...
      \begin{table}[htbp]
        \centering
        \caption{本実験での各評価の結果。MAUは、本研究で使用した動画予測モデルによる予測に対する評価、\citex{howard1990solar}は、単純差動回転モデルによるシミュレーションに対する評価を表す。}
